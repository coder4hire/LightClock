...
public class BTPacketFactory {
    ...
    public synchronized byte[] CreateScheduleUpdatePacket(ScheduleViewAdapter.ScheduleItem item) {
    ...
    public synchronized byte[] CreateSimplePacket(PacketTypes type) {
        WriteHeader(type);
        int crc = CalcCRC();
        WriteToArray(crc);
        return Obj2BytesArray(bytesArray.toArray());
    ...
    public synchronized byte[] CreateSetTimePacket(int localTime) {
    ...
    public synchronized byte[] CreateEnableScheduleItemPacket(byte itemIndex, boolean isEnabled) {
    ...
    public synchronized byte[] CreateSetManualColorPacket(int rgbw) {
    ...
    public synchronized byte[] CreateSetVolumePacket(int volume) {
    ...
    public synchronized byte[] CreateSetConfigPacket(BoardConfig cfg) {
        WriteHeader(PacketTypes.SetConfig);
        WriteToArray(cfg.Volume); // Schedule item ID
        ...
        if (calculatedCRC == ReadIntFromArray(data, HeaderSize + payloadSize)) {
            ...
            switch (packetType) {
                ...
                case ConfigRecv:
                    ...
                    cfg.Volume = data[HeaderSize]; // Schedule item ID
                    cfg.LightThresholdBack = ReadShortFromArray(data, HeaderSize+1);
                    cfg.IsStopLightEnabled = data[HeaderSize+3]!=0;
                    cfg.DoesBackligthDisableRGB = data[HeaderSize+4]!=0;
                    cfg.AreVisualConfirmationsEnabled = data[HeaderSize+5]!=0;

                    if (MainActivity.uiHandler != null) {
                        MainActivity.uiHandler.obtainMessage(MainActivity.MSG_UPDATE_CONFIG, cfg).sendToTarget();
                    }
